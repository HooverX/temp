...
def vpn_getclientvpnconf(request):
    ...
    username_config, _ = name.split('_', 1)

    if username_config != request.user.username and request.user.username != "RutokenVpn":
        return HttpResponseBadRequest()
...
def personal(request):
    ...
    username_cert, _ = name.split('_', 1)

    if username_cert != request.user.username and request.user.username != "RutokenVpn":
        return HttpResponseBadRequest()
